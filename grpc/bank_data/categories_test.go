...
func TestAPI_CategoriesNew(t *testing.T) {
	t.Run("partial", func(t *testing.T) {
		_, api := newAPI(t)
		_, err := api.CategoriesNew(apiCtx(t), categories.ReqNew_builder{
			Color: new("ffffff"),
		}.Build())
	...
	t.Run("happy", func(t *testing.T) {
		...
		resp, err := api.CategoriesNew(apiCtx(t), categories.ReqNew_builder{
			Name:  new("Name"),
			Color: new("ff00ff"),
			Icon:  new("1"),
		}.Build())
...
func TestAPI_CategoriesUpdate(t *testing.T) {
	t.Run("invalid", func(t *testing.T) {
		_, api := newAPI(t)
		_, err := api.CategoriesUpdate(apiCtx(t), categories.Category_builder{
			Id:    new("123"),
			Name:  new(""),
			Color: new("zzzz!!!zzz"),
			Icon:  new("--"),
	...
	t.Run("single", func(t *testing.T) {
		...
		resp, err := api.CategoriesList(apiCtx(t), &categories.ReqList{})
		require.NoError(t, err)
